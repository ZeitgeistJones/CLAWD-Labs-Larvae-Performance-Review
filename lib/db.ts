...
export async function initDb() {
  ...
  // Add manual_status column if it doesn't exist yet (for existing DBs)
  try {
    await sql`ALTER TABLE verdicts ADD COLUMN IF NOT EXISTS manual_status TEXT`;
  } catch {}
}
...
export async function upsertVerdict(data: {
  ...
  await sql`
    INSERT INTO verdicts (
      ...
      ${data.larvae_consensus}, ${data.idea_status}, ${data.manual_status || null},
      ${data.total_cv}, NOW(), NOW()
    )
