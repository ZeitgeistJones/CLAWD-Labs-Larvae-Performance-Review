...
export async function fetchClawdRepos() {
...
export async function fetchRepoCommits(repoName: string, limit = 10) {
  const headers: Record<string, string> = {
    Accept: "application/vnd.github+json",
  };
  if (process.env.GITHUB_TOKEN) {
  ...
  const res = await fetch(
    `https://api.github.com/repos/clawdbotatg/${repoName}/commits?per_page=${limit}`,
    { headers, next: { revalidate: 300 } }
  );
  if (!res.ok) return [];
  return res.json();
...
export async function fetchRepoReadme(repoName: string): Promise<string> {
  ...
  const res = await fetch(
    `https://api.github.com/repos/clawdbotatg/${repoName}/readme`,
